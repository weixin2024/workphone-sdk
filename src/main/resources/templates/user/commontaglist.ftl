...
<body>
		...
		<div class="row">
			...
			<div class="col-sm-9  col-md-10  main" style="padding-left: 0px;">
				<ol class="breadcrumb" style="margin-top: 3px; margin-bottom: 10px">
					...
					<li class="active">部门公共标签列表</li>
				</ol>
				<div class="row">
					<form action="/user/commontag/list" id="form" method="post">
						 
						<div class="col-md-2">
							<input type="text" name="name"
								value="${info.name!}" class="form-control"
								placeholder="标签名" aria-describedby="basic-addon1">
						</div>
                         ...
                         <#if pageBean?exists>  
                            <#list pageBean.resultList as info>
								<tr>
									...
									<td><a href="/user/commontag/toedit/${info.id}">修改</a> | <a  href="javascript:deleteInfo(${info.id})">删除</a></td>
								</tr>
							</#list>
                         </#if>  
						</tbody>
 ...
 <script type="text/javascript">
  ...
  //添加
  function toAdd(){
    location.href="/user/commontag/toadd";
  } 
  ...
  function deleteInfo(id){
	  ...
	  if(res){
	       location.href="/user/commontag/delete/"+id;
	  }
  }
