...
<body>
			...
			<div class="col-sm-9  col-md-10  main" style="padding-left: 0px;">
			    <ol class="breadcrumb" style="margin-top: 3px; margin-bottom: 10px">
					<div class="btn-group" role="group" aria-label="...">
					  <button type="button" class="btn btn-default"><a href="/user/commonterm/list" style='color: #333;' > 常用术语列表 </a></button>
					  <button type="button" class="btn btn-default"><a href="/user/commontermtype/list" style='color: #333;' > 术语分类列表 </a></button>
					</div>
				</ol>
				
				
				<ol class="breadcrumb" style="margin-top: 6px; margin-bottom: 10px">
					<li class="glyphicon glyphicon-home" aria-hidden="true"></li>
					<li class="active">部门常用术语列表</li>
				</ol>
				<div class="row">
					<form action="/user/commonterm/list" id="form" method="post">
						...
						<div class="col-md-2">
							<select class="form-control" name="tid" >
							   <option value="" <#if info?? && info.tid??> <#else> selected</#if> > -选择术语分类-  </option>
							   <#if departmentList?exists>  
					                <#list commontermTypeList as list >
							           <option value="${list.id!}" <#if list.id?? && info?? && info.tid?? && list.id == info.tid > selected</#if> > ${list.name!} </option>
					                </#list>
								</#if>  
							</select>
						</div>
                         ...
                         <#if pageBean?exists>  
                            <#list pageBean.resultList as info>
								<tr>
									...
									<td>${info.name!}</td>
									<td>
									<#if info?? && info.ctype?? && info.ctype == 1 > 文本类型</#if>
									<#if info?? && info.ctype?? && info.ctype == 2 > 图片类型</#if>
									<#if info?? && info.ctype?? && info.ctype == 3 > 视频类型</#if>
									<#if info?? && info.ctype?? && info.ctype == 4 > 语音类型</#if>
									<#if info?? && info.ctype?? && info.ctype == 5 > 其他类型</#if>
									</td>
									<td style="max-width:310px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"  title="${info.content!}">${info.content!}</td>
                                    <td>${info.create_time?string('yyyy-MM-dd HH:mm:ss')!}</td>
									<td><a href="/user/commonterm/toedit/${info.id}">修改</a> | <a  href="javascript:deleteInfo(${info.id})">删除</a></td>
								</tr>
							</#list>
                         </#if>  
						</tbody>
 ...
 <script type="text/javascript">
  ...
  //添加
  function toAdd(){
    location.href="/user/commonterm/toadd";
  } 
  ...
  function deleteInfo(id){
	  ...
	  if(res){
	       location.href="/user/commonterm/delete/"+id;
	  }
  }
