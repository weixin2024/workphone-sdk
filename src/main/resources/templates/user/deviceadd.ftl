...
<div class="panel-body">
   <div style="padding: 8px;margin: 8px" class="row">
		...
		<form novalidate="novalidate" method="post"  id="form" role="form" class="order-form">
		       <div class="col-md-12" style="height:50px">
			           ...
			           <div class="col-md-2"><span style="text-align: right;color: #666;  padding: 8px">* 所属操作员</span></div>
			           <div class="col-md-5">
							<select class="form-control" name="accountid"  id="accountid">
							   <option value="" <#if entity?? && entity.accountid??> <#else> selected</#if> > -选择所属操作员-  </option>
							   <#if userList?exists>  
					                <#list userList as info>
							           <option value="${info.id!}" <#if info.id?? && entity?? && entity.accountid?? && info.id == entity.accountid > selected</#if> > ${info.account!} </option>
					                </#list>
								</#if>  
							</select>
						</div>
			    ...
			    <div class="col-md-12" style="height:50px">
					   ...
					   <div class="col-md-5">
					     <input type="text" value="${entity.devnickname!}" name="devnickname" aria-describedby="basic-addon1" placeholder="填写手机名" class="form-control"> 
					   </div>
					   <div class="col-md-3"></div>
			   </div>
			    <div class="col-md-12" style="height:50px">
			   ...
			   <div style="padding: 8px;padding-bottom: 10px;margin: 8px">
			        <div style="text-align: center;" class="col-md-12">
			           ...
			           <button   type="button" onclick="javascript:history.back()" class="btn btn-default">返回上一层</button>
				    </div>
			   </div>
 ...
 <script>
 function submitForm(){ 
     if($('#deviceid').val() && $('#accountid').val()){
		 var targetUrl ="";
		 if($('#id').val()){
	         targetUrl = "/user/device/edit"; 
	     }else{
	         targetUrl = "/user/device/add"; 
	     }
	    ...
	    $("#flabel").html("<font color='red'>设备号、部门不能为空!</font>");
	 }
