...
public class FileUtil {
	...
	private final static String LOCAL_PATH = "/home/data";
	private final static String FILE_PATH = "/juliao";
	private final static String publicUrl = "http://8.210.165.20:13086";//这里IP需要改成你自己的
	 ...
	 * 
	 */
	public static Map<String, Object> saveFile(MultipartFile file, String isMp3ToAmr,String isAmrToMp3) throws Exception {
		...
		final String newPath = LOCAL_PATH + FILE_PATH + File.separator + datePath + File.separator + md5 + "." + suffix;
		...
		if (!StringUtils.isEmpty(isMp3ToAmr) && isMp3ToAmr.equals("true")) {
			...
			if (suffix.equals("mp3") || suffix.equals("MP3")) {
				try {
					new Thread(new Runnable() {
						@Override
						public void run() {
							ConverterAmrAndMp3.converterEncoderAmr(null, newPath);
						}
					}).start();
					suffix = "amr";
				} catch (Exception e) {
			...
			if (!StringUtils.isEmpty(isAmrToMp3) && isAmrToMp3.equals("true")) {
				// 如果是amr
				if (suffix.equals("amr") || suffix.equals("AMR")) {
					try {
						new Thread(new Runnable() {
							@Override
							public void run() {
								ConverterAmrAndMp3.converterDecoderMp3(null, newPath);
							}
						}).start();
					} catch (Exception e) {
						e.printStackTrace();
