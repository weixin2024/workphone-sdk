...
@Service
public class GetWechatFriendDetailWebsocketHandler{
    ...
    public  void handleMsg(ChannelHandlerContext ctx ,TransportMessage vo, String contentJsonStr) {
        try {
        	...
        	if(null != req){
        		WxContactInfo info  = weChatContactService.findContactinfoByfriendid(req.getCid(), req.getWechatid(),req.getFriendid());
    			String json = null ;
    			if(null != info){
    			     json = JSON.toJSONString(info);
    			}
        		//3、告诉PC客户端消息已收到
    			MessageUtil.sendCustomJsonMsg(ctx, "GetWechatFriendDetailResp", json);
        	}else{
